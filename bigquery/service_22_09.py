...
def run_prediction(row, price):
    ...
    for v in spikes:
        ...
        prev = v
    custom = 0
    drops = price - 2 < (row['prices_list'][1] + row['prices_list'][2] + row['prices_list'][3]) / 3 and price < \
            row['prices_list'][1] and min(row['prices_list'][:25]) + 30 < max(row['prices_list'][:25])
    grows = price + 2 > (row['prices_list'][1] + row['prices_list'][2] + row['prices_list'][3]) / 3 and price > \
            row['prices_list'][1] and min(row['prices_list'][:25]) + 30 < max(row['prices_list'][:25])
    if grows:
        custom += 10
    if drops:
